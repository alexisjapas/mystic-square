...
class Agent(threading.Thread):
    ...
    def __init__(self, current_pos, target_pos, color, positions, lock):
        super().__init__()
    ...
    def has_reach_target(self):
        return not any([self.target_pos[i] - self.current_pos[i] for i in range(len(self.current_pos))])

    def is_position_valid(self, position):
        return all([position[i] >= 0 and position[i] < Agent.grid_dim for i in range(len(position))])

    def compute_path_to_target(self):
        direction = None
        diff_tuple = tuple(self.target_pos[i] - self.current_pos[i] for i in range(len(self.target_pos)))
        non_zero_indexes = [i for i in range(len(diff_tuple)) if diff_tuple[i] != 0 or self.err_rate > uniform(0, 1)]
        if non_zero_indexes:
    ...
    def run(self):
        while True:
            ...
            if direction:
                with self.lock:
                    new_position = tuple(self.current_pos[i] + direction[i] for i in range(len(direction)))
